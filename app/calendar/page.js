...
const renderTopSection = () => {
  return (
    <Flex justifyContent="space-between" mb={4} alignItems="center">
      <Text fontSize="2xl" fontWeight="bold" p={2} border="1px" borderColor="teal" borderRadius="md">
        Manage Your Events
      </Text>
...
const renderDateFilterSection = () => {
  return (
    <Flex justifyContent="space-between" alignItems="center" mb={4}>
      <Text fontSize="3xl" fontWeight="bold" p={2} border="1px" borderColor="teal" borderRadius="md">
        {getFormattedMonthYear(year, month)}
      </Text>
      <Flex>
        <Select placeholder="Filter by organization">
          {Array.from(new Set(mockEvents.map(event => event.organization))).map(org => (
...
const generateWeekDays = () => {
  return weekDays.map((day, index) => (
    ...
    const generateDays = () => {
      ...
      for (let i = 0; i < startingDay; i++) {
          days.push(
              <Flex flex="1" key={`b-${i}`} border="1px" borderColor="gray.200" p={2} align="center" justify="center" />
          );
      }
  ...
  for (let day = 1; day <= daysInMonth; day++) {
    ...
    days.push(
      <Flex key={day} flex="1" border="1px" borderColor="gray.200" p={2} direction="column" position="relative">
        <Text position="absolute" top="1" right="2" fontSize="sm" fontWeight="bold">
          {day}
        </Text>
        {mockEvents.filter(event => isSameDay(dayDate, new Date(event.startDate))).map((event, index) => (
          <Box key={index} className="event" p={2} mt={5} borderRadius={4} bg="gray.100" textAlign="center">
...
for (let nextDay = 1; nextDay <= nextMonthDaysNeeded; nextDay++) {
  days.push(
    <Flex key={`n-${nextDay}`} flex="1" border="1px" borderColor="gray.200" p={2} align="center" justify="center">
      <Text 
        position="absolute" 
        top="2" 
        right="2" 
    ...
    weeks.push(
        <Flex key={i} wrap="wrap" bg="white">
            {days.slice(i, i + 7).map((dayComponent, index) => (
                <Flex key={index} flexGrow={1} flexBasis="14.28%" minHeight="130px" border="1px" borderColor="gray.300" p={2} position="relative">
                    {dayComponent}
                </Flex>
            ))}
        </Flex>
...
return (
  <Box maxWidth="1400px" m="auto" p={10} bg="white" borderRadius="lg">
    ...
    <Box borderRadius="lg" border="1px" borderColor="gray.200" p={0}> {/* Increased padding */}
      <Flex wrap="wrap">
        {generateWeekDays()}
      </Flex>
      {generateDays()}
